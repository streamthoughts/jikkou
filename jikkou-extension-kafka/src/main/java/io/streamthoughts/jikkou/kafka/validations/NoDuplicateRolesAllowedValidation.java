...
@EnableAutoConfigure
public class NoDuplicateRolesAllowedValidation implements ResourceValidation {
    ...
    public void validate(final @NotNull HasMetadata resource) throws ValidationException {
        ...
        final Collection<V1KafkaAccessRoleObject> roles = accessList.getSpec().getRoles();

        if (roles.isEmpty()) return;

        final Map<String, List<V1KafkaAccessRoleObject>> groupedByName = Nameable.groupByName(roles);
