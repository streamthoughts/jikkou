...
public class QuotasEntityValidation extends QuotasValidation {
     ...
     */
    @Override
    public void validateQuota(@NotNull V1KafkaQuotaObject quota) throws ValidationException {
        QuotaType type = quota.getType();
        if (type == null) {
            throw new ValidationException("quota type is null", this);
        }
        try {

            type.validate(quota.getEntity());
        } catch (Exception e) {
