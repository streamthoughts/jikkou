...
* As a zip/tar.gz package from https://github.com/streamthoughts/jikkou/releases/tag/v0.11.0[GitHub Releases]
* As a fatJar available from https://repo.maven.apache.org/maven2/io/streamthoughts/jikkou/0.11.0/[Maven Central]
* As a docker image available from https://hub.docker.com/r/streamthoughts/jikkou[Docker Hub].
* As a Debian package from https://github.com/streamthoughts/jikkou/releases/tag/v0.11.0[GitHub Releases]

```bash
wget https://github.com/streamthoughts/jikkou/releases/download/v0.11.0/jikkou.deb
...
* `apiVersion`: the version of the Jikkou's API (default is `1`).
...
* `template.values`: a set of key/value pairs that
...
Jikkou supports a simple templating mechanism using https://jinja.palletsprojects.com/en/3.0.x/[Jinja] notation for allowing you to reuse the same _resource files_ for multiple target environments.

Jikkou defines a number of top-level objects that are passed to the template engine and that you can access in your templates.

* `values`: Values passed into the template through the command-line arguments `--values-files` or `--set-value`. By default, `Values` is empty.
...
* `labels`: Labels passed into the template through the command-line argument: `--set-label`. By default, `Labels` is empty.
    * In addition, values can be directly into the template file using the property `metadata.labels`.
...
[source, bash]
----
jikkou validate --files kafka-topics.yml --values-files=values.yml
----
...
spec:
    ...
    topics:
    - name: "{{ values.topic_prefix }}my-first-topic"
      partitions: 6
      replication_factor: "{{ values.default_replication_factor }}"
      config_map_refs: [ 'DefaultTopicConfig' ]

    - name: "{{ values.topic_prefix }}my-second-topic"
      partitions: 12
      replication_factor: "{{ values.default_replication_factor }}"
      config_map_refs: [ 'DefaultTopicConfig' ]
----

Jikkou will perform two rendering of your template.
A first time using only the `values` and `labels` passed through the command-lines arguments. Then, a second time after parsing the YAML resource file using also the `values` and `labels` as defined into the resource file.
Thus, it is important that, after the first rendering, your resource file is converted into a valid YAML file.

In addition, you should use `{% raw %}...{% endraw %}` tags to ensure that variables defined into the template are not be interpreted during the first rendering.
...
public void setUp() {
    ...
    var processor = new DefaultProcessorFactory(new ExtensionRegistry()).create(config);
