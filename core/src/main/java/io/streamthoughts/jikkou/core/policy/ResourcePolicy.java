...
public final class ResourcePolicy {
     ...
     */
    public boolean canAccept(final Resource resource) {
        MatchResources matchResources = this.policy.getSpec().matchResources();

        boolean canAccept = true;

        List<String> acceptedApiVersion = matchResources.resourceSelector().apiVersions();
        if (acceptedApiVersion != null && !acceptedApiVersion.isEmpty()) {
            canAccept = acceptedApiVersion.contains(resource.getApiVersion());
        }

        List<String> acceptedKinds = matchResources.resourceSelector().kinds();
        if (acceptedKinds != null && !acceptedKinds.isEmpty()) {
            canAccept = acceptedKinds.contains(resource.getKind());
        }
