...
import io.jikkou.schema.registry.api.data.ErrorResponse;
import io.jikkou.schema.registry.api.data.ModeObject;
import io.jikkou.schema.registry.api.data.SubjectSchemaReference;
...
import io.jikkou.schema.registry.change.SchemaSubjectChangeOptions;
...
public abstract class AbstractSchemaSubjectChangeHandler implements ChangeHandler {
    ...
    protected Mono<Void> updateMode(final ResourceChange change) {
    ...
    protected Mono<Void> deleteMode(@NotNull ResourceChange change) {
        final String subject = change.getMetadata().getName();
        if (LOG.isInfoEnabled()) {
