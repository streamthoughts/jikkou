...
public class ApplyConfigMapsTransformation implements Transformation {
    ...
    @Override
    public @NotNull V1SpecsObject transform(@NotNull V1SpecsObject specs) {
        final List<V1TopicObject> topics = specs.topics()
                .stream()
                .map(topic -> topic.apply(specs.configMaps()))
                .collect(Collectors.toList());

        return specs.topics(topics);
    }
}
